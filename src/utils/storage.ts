...
export interface SectionSubmission {
  ...
  marks?: Record<number | string, 'correct' | 'incorrect' | number | null>;
  submittedAt: string;
  timeSpent: string;
...
export interface ExamSubmission {
  ...
  // NEW: Writing band score for partial writing tests (calculated from Task 1 & 2)
  writingBandScore?: number;  // Final band score (0-9) for partial writing tests
...
const getSubmissionPath = (trackId: string, examCode: string, submissionId: string): string => {
  // 🔴 BUG FIX #4: Validate path components
  if (!trackId || !examCode || !submissionId) {
    throw new Error(`Invalid submission path: trackId='${trackId}', examCode='${examCode}', submissionId='${submissionId}'`);
  }
...
const localStorageHelper = {
  ...
  add(submission: ExamSubmission): void {
    ...
    // ✅ DEDUP FIX: Check if submission with this ID already exists
    const existingIndex = submissions.findIndex(s => s.id === submission.id);
    
    if (existingIndex !== -1) {
      // Replace existing submission instead of creating duplicate
      console.warn(
        `⚠️ Submission ID "${submission.id}" already exists in localStorage. ` +
        `Updating existing record instead of creating duplicate.`
      );
...
export const storage = {
  ...
  async getSubmissions(): Promise<ExamSubmission[]> {
    try {
      ...
      const seenIds = new Set<string>(); // ✅ DEDUP FIX: Track unique submission IDs

      // 🔴 BUG FIX #2: Add null-safety checks to prevent crashes
      ...
      Object.keys(data).forEach(trackId => {
        ...
        if (!trackData || typeof trackData !== 'object') {
          console.warn(`⚠️ Invalid trackData for trackId='${trackId}':`, trackData);
        ...
        Object.keys(trackData).forEach(examCode => {
          ...
          if (!examData || typeof examData !== 'object') {
            console.warn(`⚠️ Invalid examData for trackId='${trackId}', examCode='${examCode}':`, examData);
            return; // Skip this exam
          }
          
          Object.keys(examData).forEach(submissionId => {
            // Skip metadata entries and empty keys (from empty examCode bug)
            if (submissionId === '_metadata' || submissionId === '') {
              console.warn(`⚠️ Skipping invalid submissionId='${submissionId}' for trackId='${trackId}', examCode='${examCode}'`);
              return;
            }
            
            const submission = examData[submissionId];
            
            // Validate submission has required fields
            if (!submission || typeof submission !== 'object' || !submission.id) {
              console.warn(`⚠️ Invalid submission object:`, { submissionId, submission });
              return; // Skip this submission
            }
            
            // ✅ DEDUP FIX: Only add if we haven't seen this submission ID before
            if (!seenIds.has(submission.id)) {
              seenIds.add(submission.id);
              submissions.push(submission);
            } else {
              console.warn(
                `⚠️ Duplicate submission detected: "${submission.id}". Skipping to prevent duplication in results.`
              );
            }
          });
        });
      ...
      console.log(`✓ Successfully loaded ${submissions.length} submissions from Firebase`);
      ...
      // Fallback to localStorage
      console.log('⚠️ Falling back to localStorage due to Firebase error');
      return localStorageHelper.getAll();
    }
  },
  ...
  async addSubmission(submission: ExamSubmission): Promise<boolean> {
    try {
      ...
      // 🔴 BUG FIX #3: Validate BEFORE saving to localStorage
      if (!submission.trackId || !submission.examCode) {
        console.error('❌ CRITICAL: Cannot save submission - Missing required fields!');
        console.error('Track ID:', submission.trackId);
        console.error('Exam Code:', submission.examCode);
        console.error('Full submission:', submission);
        // Don't save to localStorage with invalid data!
        return false;
      }
      ...
      let path = '';
      try {
        path = getSubmissionPath(submission.trackId, submission.examCode, submission.id);
      } catch (pathError) {
        console.error('❌ Cannot create Firebase path:', pathError);
        // Already in localStorage, but can't sync to Firebase
        return true;
      }
  ...
  async updateMark(submissionId: string, questionKey: number | string, mark: 'correct' | 'incorrect' | number | null): Promise<boolean> {
    try {
      // Update localStorage first
  ...
  async publishResult(submissionId: string): Promise<boolean> {
    try {
      ...
      if (isWriting) {
        // For writing tracks, use fixed task keys
        ...
        taskKeys.forEach(taskKey => {
          ...
          // A valid band score is a number between 0-9
          if (mark !== null && mark !== undefined && typeof mark === 'number') {
            markedCount++;
            correctCount++;  // For writing tasks, if it's marked with a band score, count it
          }
        });
      } else {
      ...
      // For writing tests, calculate final band score instead of manualScore
      if (isWriting) {
        const task1Score = submission.marks!['task1'];
        const task2Score = submission.marks!['task2'];
        
        if (typeof task1Score === 'number' && typeof task2Score === 'number') {
          // IELTS formula: (Task 1 + Task 2 × 2) ÷ 3
          const rawAverage = (task1Score + task2Score * 2) / 3;
          // Round to nearest 0.5
          const finalBandScore = Math.round(rawAverage * 2) / 2;
          
          // Store as manualScore (percentage conversion for compatibility)
          // OR better: store the actual band score
          updates.manualScore = Math.round((finalBandScore / 9) * 100);
          updates.writingBandScore = finalBandScore;
          
          console.log('Writing test result published:', { 
            task1Score, 
            task2Score, 
            finalBandScore, 
        ...
        // For reading/listening tests, calculate from correct answers
        updates.manualScore = Math.round((correctCount / expectedCount) * 100);
      }
  ...
  subscribeToSubmissions(callback: (submissions: ExamSubmission[]) => void): Unsubscribe {
    ...
    return onValue(submissionsRef, (snapshot) => {
      ...
      if (snapshot.exists()) {
        ...
        // 🔴 BUG FIX: Add null-safety checks to prevent partial data from incomplete snapshots
        ...
        Object.keys(data).forEach(trackId => {
          ...
          // Check if trackData is valid
          if (!trackData || typeof trackData !== 'object') {
            console.warn(`⚠️ Invalid trackData in listener for trackId='${trackId}':`, trackData);
            return; // Skip this track
          }
          ...
          Object.keys(trackData).forEach(examCode => {
            ...
            // Check if examData is valid
            if (!examData || typeof examData !== 'object') {
              console.warn(`⚠️ Invalid examData in listener for trackId='${trackId}', examCode='${examCode}':`, examData);
              return; // Skip this exam
            }
            
            Object.keys(examData).forEach(submissionId => {
              // Skip metadata entries and empty keys
              if (submissionId === '_metadata' || submissionId === '') {
                return;
              }
              ...
              // Validate submission has required fields
              if (!submission || typeof submission !== 'object' || !submission.id) {
                console.warn(`⚠️ Invalid submission in listener:`, { submissionId, submission });
                return; // Skip this submission
              }
  ...
  subscribeToTrackSubmissions(trackId: string, callback: (submissions: ExamSubmission[]) => void): Unsubscribe {
    ...
    return onValue(trackRef, (snapshot) => {
      ...
      if (snapshot.exists()) {
        ...
        // 🔴 BUG FIX: Add null-safety checks
        if (trackData && typeof trackData === 'object') {
          Object.keys(trackData).forEach(examCode => {
            const examData = trackData[examCode];
            ...
            // Check if examData is valid
            if (!examData || typeof examData !== 'object') {
              console.warn(`⚠️ Invalid examData in track listener for trackId='${trackId}', examCode='${examCode}':`, examData);
              return;
            }
            
            Object.keys(examData).forEach(submissionId => {
              // Skip metadata entries and empty keys
              if (submissionId === '_metadata' || submissionId === '') return;
      ...
      if (snapshot.exists()) {
        ...
        if (examData && typeof examData === 'object') {
          Object.keys(examData).forEach(submissionId => {
            ...
            // Validate submission (only require id field)
            if (!submission || typeof submission !== 'object' || !submission.id) {
              return;
            }
